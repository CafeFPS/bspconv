...
enum lumptype_t
{
    ...
    LUMP_MODELS = 0x000E,
    LUMP_TEXDATA_STRING_DATA = 0x000F,
    ...
    LUMP_ENTITY_PARTITIONS = 0x0018, // 24
...
struct lump_t
{
    ...
    bool operator<(lump_t& right) const
    {
        return fileofs < right.fileofs;
    }
};
...
struct BSPHeader_t
{
    ...
    /* 0x0004 */ short version;
    /* 0x0006 */ short flags;

    /* 0x0008 */ int mapRevision;
    /* 0x000c */ int lastLump;
    /* 0x0010 */ lump_t lumps[LUMP_COUNT]; // size: 16 * 128
};

enum GameLumpId_t
{
    GAMELUMP_DETAIL_PROPS = 1685090928, //'dprp',
    GAMELUMP_DETAIL_PROP_LIGHTING = 1685089396, //'dplt',
    GAMELUMP_STATIC_PROPS = 1936749168, //'sprp',
    GAMELUMP_DETAIL_PROP_LIGHTING_HDR = 1685089384, //'dplh',
};
...
struct dentitypartitionheader_t
{
    enum
    {
        VERSION = '10'
...
struct dlightprobe_t
{
    ...
    char pad[4]; // removed in later bsp versions
};
...
namespace r5
{
    ...
    namespace v8
    {
        struct dbrushmodel_t
        {
            r5::mstudiocollmodel_t model;
            r5::v8::mstudiocollheader_t header;
        };
    }
    ...
    namespace v121
    {
        ...
        {
            r5::mstudiocollmodel_t model;
            r5::v121::mstudiocollheader_t header;
        };
    }

    namespace v51
    {
        // this might not have been introduced in v51 but since we only support v51 rn, it might as well go here
        struct dlightprobe_t
        {
            short ambientSH[12];
            short skyDirSunVis[4];
